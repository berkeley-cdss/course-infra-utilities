...
BCOURSES_ID = TTLCache(1000, 1800) # cache mapping domains to bcourses id
...
def get_course(domain=None):
    ...
    if not domain:
        domain = get_domain()
    if "pr" in domain: # if is pull request
...
def get_bcourses_id(domain=None) -> int:
    if getenv("ENV") == "DEV":
        return 157 # This is for local testing on bCourses test instance (mango 101 course ID)
    if not domain:
        domain = get_domain()
    if "pr" in domain: # if is pull request
        BCOURSES_ID[domain] = 1521247 # This is a bCourses ID Silas is assigned the teacher role
    if domain not in BCOURSES_ID:
        BCOURSES_ID[domain] = auth.get_bcourses_id(domain=domain)
    return BCOURSES_ID[domain]
