...
def sh(
    *args,
    ...
    elif capture_output and out.returncode:
        stdout_output = out.stdout.decode("utf-8", "replace")
        stderr_output = out.stderr.decode("utf-8", "replace")
        if len(stdout_output) > 1500:
            print(stdout_output[:500]
                + f"...\n...({len(stdout_output) - 1000} characters omitted)...\n..."
                + stdout_output[-500:], file=sys.stdout)
        else:
            print(stdout_output, file=sys.stdout)
        if len(stderr_output) > 1500:
            print(stderr_output[:500]
                + f"...\n...({len(stderr_output) - 1000} characters omitted)...\n..."
...
def tmp_directory(*, clean=False):
    main_dir = os.getcwd()
    if clean:
        print("CREATING TEMP DIR", file=sys.stderr)
